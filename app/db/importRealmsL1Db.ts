...
async function getRealms(first: number, skip: number) {
  const response = await fetch(subgraphUrl, {
    ...
    body: JSON.stringify({
      ...
      query: `
          query getRealms {
              realms(first: ${first}, skip: ${skip}, orderBy:id, orderDirection:asc) {
                  id
                  rarityScore
                  rarityRank
...
async function main() {
  ...
  for (let skip = 0; skip < 8000; skip += first) {
    ...
    const realms = (await getRealms(1000, 0)).map((realm: any) => {
      return {
        ...realm,
        ...traitsDb[realm.id]
      };
